...
The first 4 bits, `0011` store rotation data or adjacency data. We can do that with `entry & 15`, which is `0011`. `0011` is 3, so that's the image we use from `tileset[tileId].images`, which we then render on the canvas every frame.

...
async function serveFile(req: Request, filePath: string) {
  ...
  if (auth?.signedIn) {
    const userRows =
      await sql`SELECT preferred_theme from users where id = ${auth.user}`;
    if (
      userRows.length > 0 &&
      userRows[0].preferred_theme !== "" &&
      userRows[0].preferred_theme !== null
    ) {
...
const DATABASE_URL =
  ...
  "postgres://postgres:postgres@localhost:4321/postgres";
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    const corsOrigin = ALLOWED_ORIGINS.includes(origin as string)
      ? origin
      : "null";
    const CORS = {
    ...
    if (pathname === "/api/oauth" && req.method === "POST") {
      ...
      if (provider === "hack-club") {
        ...
        if (!tokenResponse.ok) {
          return new Response("Failed to authenticate with Hack Club", {
            status: 401,
          });
        }
        ...
        const existingUserRows =
          await sql`SELECT id FROM users WHERE username = ${response.user.profile.display_name} LIMIT 1`;
        ...
        headers.append(
          "Set-Cookie",
          `session-id=${sessionId[0].id}; Path=/; MaxAge=${60 * 60 * 24 * 14}`,
        );
        headers.append(
          "Set-Cookie",
          `token=${uuid}; Path=/; MaxAge=${60 * 60 * 24 * 14}`,
        );
        return new Response(JSON.stringify({ id: sessionId[0].id }), {
          status: 200,
          headers: headers,
        });
      }
    ...
    if (pathname == "/api/login" && req.method == "POST") {
      try {
        ...
        const rows =
          await sql`select id, password_hash from users where username = ${username} limit 1`;
        const user = rows[0];
        if (!user || !user.id)
          return new Response(
            "Username does not exist",
            withCors({ status: 404 }, CORS),
          );
        ...
        headers.append(
          "Set-Cookie",
          `session-id=${sessionId[0].id}; Path=/; SameSite=Lax; MaxAge=${60 * 60 * 24 * 14}`,
        );
        headers.append(
          "Set-Cookie",
          `token=${uuid}; Path=/; SameSite=Lax; MaxAge=${60 * 60 * 24 * 14}`,
        );

        return new Response(
          JSON.stringify({ id: user.id }),
          withCors({ status: 200, headers: headers }, CORS),
        );
      } catch (e) {
        console.error(e);
        return new Response("Server Error", withCors({ status: 500 }, CORS));
      }
    }
    ...
    if (pathname == "/api/level") {
      ...
      if (levelId) {
        ...
        return new Response(
          JSON.stringify({
            error: "Must specify a level id with the levelId parameter",
          }),
          withCors(
            { status: 404, headers: { "Content-Type": "application/json" } },
            CORS,
    ...
    if (pathname == "/api/browse") {
      ...
      const pageSize = 50;
      const totalCountRes =
        await sql`select count(*)::int as total from levels where public = true`;
      const total = totalCountRes[0].total ?? 0;
      const pages = Math.ceil(total / pageSize);

      const levels =
        await sql`select id, data, name, created_at, width, height, owner, tags, image_url, approvals, disapprovals, approval_percentage, total_plays, finished_plays, description, level_style from levels
        WHERE public = true 
        ${sortBySQL}
      ...
      for (const level of levels) {
        if (usernames.has(level.owner)) {
          ...
          const username =
            await sql`select username from users where id = ${level.owner}`;
    ...
    if (pathname == "/api/upload" && req.method == "POST") {
      ...
      if (authentication?.signedIn) {
        ...
        `;
        return new Response(
          JSON.stringify({ levelId: insertInto[0].id }),
          withCors({ status: 200 }, CORS),
        );
      } else {
    ...
    if (pathname == "/api/myLevels" && req.method == "GET") {
      ...
      if (authentication?.signedIn) {
        const level =
          await sql`select id, data, name, width, height, owner, tags, image_url, approvals, disapprovals, approval_percentage, total_plays, finished_plays, description, level_style from levels where owner = ${authentication.user}`;
        if (!level[0] || level.length === 0) {
          return new Response(
            JSON.stringify({ error: "Level not found" }),
            withCors(
    ...
    if (pathname == "/api/rate") {
      ...
      if (authentication?.signedIn) {
        const rated =
          await sql`select id, thumbs_up from ratings where user_id = ${authentication.user} AND level_id = ${levelId}`;
    ...
    try {
      ...
      return new Response(
        file,
        withCors({ status: 200, headers: { "Content-Type": mime } }, CORS),
      );
    } catch {
      return new Response("Not Found", withCors({ status: 404 }, CORS));
    }
